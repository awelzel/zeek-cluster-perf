# All workers publish via publish_hrw() the potential_scanner() event
# at a fixed rate to proxies.
...
event do_publish_tick() {
	...
	while ( i < publishes_per_tick ) {
		...
		Cluster::publish_hrw(Cluster::proxy_pool, subx, Scan::potential_scanner, dummy_ci, T, F, "filtrator" + subx + cat(total));
	}
}

event tick() {
	if ( total >= total_publishes ) {
		# Finished, let the manager know. For ZeroMQ this
		# is going via the same XPUB socket as messages
		# to the proxies.
...
event Cluster::Bench::test_terminate() {
	...
	local sum_expected = total_publishes * Cluster::Bench::workers_total;
